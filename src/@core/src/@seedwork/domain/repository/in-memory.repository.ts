...
import { RepositoryInterface, SearchParams, SearchResult, SearchableRepositoryInterface } from "./repository-contract";
...
export abstract class InMemorySearchableRepository<E extends Entity> extends InMemoryRepository<E> implements SearchableRepositoryInterface<Entity> {
  ...
  protected async applySort(items: E[], sort: string | null, sort_dir: string | null): Promise<E[]> {
    if (!sort || !sort_dir || !this.sortableFields.includes(sort)) {
      return items;
    }
